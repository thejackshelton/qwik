...
A common problem in front-end development is creating a modal dialog popup from a component. This is because the modal dialog needs to be rendered in a different part of the DOM tree, and the component that triggers the modal needs a way to affect the location of rendering.

In other frameworks, this is often solved by using a dedicated API such as [`createPortal()`](https://react.dev/reference/react-dom/createPortal). However such APIs don't work well with server-side rendering, and so an alternative approach is needed.
...
1. We get a hold of the `PopupManager` API through: 
   ```tsx
...
3. Finally we can use our `popupManager` API to hide the popup:
   ```tsx
...
## Adding `PopupManager` to root `layout.tsx`
